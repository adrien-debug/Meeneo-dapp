        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#9EB3A8]/10">
          ...
          <div className="flex gap-3">
            <button
              ...
              onClick={() => exportAsCSV(rows as Record<string, unknown>[], `${exportName}.csv`)}
            >
              CSV
            </button>
            <button
          ...
          <thead className="sticky top-0 bg-[#F2F2F2] shadow-sm">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.key}
                  className="px-3 py-2 text-left text-[10px] font-semibold text-[#9EB3A8] uppercase tracking-wider whitespace-nowrap"
                >
                  {col.label}
                </th>
              ))}
            ...
            {rows.map((row, i) => (
              <tr key={i} className="hover:bg-[#F2F2F2]/50 transition-colors">
                {columns.map((col) => (
                  <td
                    key={col.key}
                    className="px-3 py-1.5 text-[#0E0F0F] font-medium whitespace-nowrap"
                  >
