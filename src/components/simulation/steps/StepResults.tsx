        ...
        <div className="space-y-6">
          ...
          {(() => {
            const hasCommercial = scenarios.some(
              (s) => runData.scenario_results[s]?.commercial?.total_commercial_value_usd > 0,
            )
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-[#9EB3A8]/10">
                    <tr>
                      ...
                      {scenarios.map((s) => (
                        <td key={s} className="px-3 py-2 font-mono text-[#0E0F0F]">
                          {formatUSD(
                            runData.scenario_results[s]?.aggregated?.metrics?.final_portfolio_usd ||
                              0,
                          )}
                        </td>
                      ))}
                      ...
                      <tr className="bg-[#F2F2F2]/50">
                        ...
                        {scenarios.map((s) => (
                          <td key={s} className="px-3 py-2 font-mono text-[#9EB3A8]">
                            {formatUSD(
                              runData.scenario_results[s]?.aggregated?.metrics
                                ?.gross_final_portfolio_usd || 0,
                            )}
                          </td>
                        ))}
                      </tr>
                    ...
                    <tr>
                      ...
                      {scenarios.map((s) => {
                        const pct =
                          runData.scenario_results[s]?.aggregated?.metrics?.total_return_pct || 0
                        return (
                          <td
                            key={s}
                    ...
                    <tr>
                      ...
                      {scenarios.map((s) => (
                        <td key={s} className="px-3 py-2 font-mono text-[#0E0F0F]">
                          {formatPercent(
                            runData.scenario_results[s]?.aggregated?.metrics?.effective_apr || 0,
                          )}
                        </td>
                      ))}
                    </tr>
                    ...
                    {hasCommercial && (
                      <tr className="bg-amber-50">
                        <td className="px-3 py-2 font-medium text-[#E8A838]">
                          Commercial Fees (Total)
                        </td>
                        {scenarios.map((s) => (
                          <td key={s} className="px-3 py-2 font-mono text-[#E8A838]">
                            {formatUSD(
                              runData.scenario_results[s]?.commercial?.total_commercial_value_usd ||
                                0,
                            )}
                          </td>
                        ))}
                ...
                {scenarios.map((s) => {
                  ...
                  return (
                    <div key={s} className="space-y-3">
                      ...
                      <SimMetric
                        label="Effective APR"
                        value={formatPercent(yb?.effective_apr || 0)}
                      />
                    </div>
                  )
                })}
              ...
              <div className={`${CARD} p-4`}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart
                    data={(() => {
                      const baseYield =
                        runData.scenario_results[scenarios[0]]?.yield_bucket?.monthly_data || []
                      return baseYield.map((_: any, t: number) => {
                        const row: any = { month: t }
                        for (const s of scenarios) {
                          row[s] =
                            runData.scenario_results[s]?.yield_bucket?.monthly_data?.[t]
                              ?.cumulative_yield_usd || 0
                        }
                        return row
                      })
                    })()}
              ...
              <div className={`${CARD} p-4`}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart
                    data={(() => {
                      const baseHolding =
                        runData.scenario_results[scenarios[0]]?.btc_holding_bucket?.monthly_data ||
                        []
                      return baseHolding.map((_: any, t: number) => {
                        const row: any = { month: t }
                        for (const s of scenarios) {
                          row[s] =
                            runData.scenario_results[s]?.btc_holding_bucket?.monthly_data?.[t]
                              ?.bucket_value_usd || 0
                        }
                        return row
                      })
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {scenarios.map((s) => {
                  ...
                  const holdingHit =
                    runData.scenario_results[s]?.btc_holding_bucket?.metrics?.target_hit
                  return (
                    <div key={s} className="space-y-3">
                      <h4
                        className="text-xs font-semibold uppercase"
                      ...
                      <SimMetric
                        label="Effective APR"
                        value={formatPercent(mb?.effective_apr || 0)}
                      />
                      <SimMetric
                        label="OPEX Coverage"
              ...
              <div className={`${CARD} p-4`}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart
                    data={(() => {
                      const baseWaterfall =
                        runData.scenario_results[scenarios[0]]?.mining_bucket?.monthly_waterfall ||
                        []
                      return baseWaterfall.map((_: any, t: number) => {
                        const row: any = { month: t }
                        for (const s of scenarios) {
                          row[s] =
                            runData.scenario_results[s]?.mining_bucket?.monthly_waterfall?.[t]
                              ?.health_score || 0
                        }
                        return row
                      })
          ...
          {viewTab === 'btc_mgmt' &&
            (() => {
              const activeScenario = scenarios.includes(waterfallScenario)
                ...
                : scenarios[0]
              const btcMgmt: any[] =
                runData.scenario_results[activeScenario]?.aggregated?.btc_under_management || []
              const btcMgmtMetrics =
                runData.scenario_results[activeScenario]?.aggregated
                  ?.btc_under_management_metrics || {}
              const holdingMetrics =
                runData.scenario_results[activeScenario]?.btc_holding_bucket?.metrics || {}
                ...
                <div className="space-y-5">
                  ...
                  <div className={`${CARD} p-4`}>
                    ...
                    <div className="text-[11px] text-[#9EB3A8] space-y-1 leading-relaxed">
                      <p>
                        This view tracks all BTC held across the product, showing how its $ value
                        appreciates over time:
                      </p>
                      <ol className="list-decimal list-inside space-y-0.5 pl-2">
                        <li>
                          <span className="text-cyan-600 font-medium">BTC Holding Bucket</span> —
                          BTC purchased for capital reconstitution (held until target price is
                          struck)
                        </li>
                        <li>
                          <span className="text-[#E8A838] font-medium">Mining Capitalization</span>{' '}
                          — Surplus BTC accumulated from mining after OPEX and yield
                        </li>
                      </ol>
                      <p className="mt-2">
                        When the target price is{' '}
                        <span className="text-green-600 font-medium">struck</span>, BTC from the
                        Holding bucket is sold for capital reconstitution. The remaining BTC (mining
                  ...
                  <div className={`${CARD} p-4`}>
                    ...
                    <p className="text-[10px] text-[#9EB3A8] mt-1">
                      As BTC price (purple line) increases, the $ value of BTC held appreciates.
                      This creates yield-generating capability beyond the initial investment.
                    </p>
                  </div>
                  ...
                  {holdingMetrics.btc_quantity > 0 && (
                    <div className={`${CARD} p-4`}>
                      ...
                      <p className="text-[10px] text-[#9EB3A8] mt-1">
                        Shows how much the holding bucket BTC has appreciated compared to the
                        purchase price of {formatUSD(holdingMetrics.buying_price_usd || 0)}/BTC.
                      </p>
                    </div>
                  )}
                  ...
                  <div className={`${CARD} overflow-hidden`}>
                        ...
                        <tbody className="divide-y divide-[#9EB3A8]/5">
                          {btcMgmt.map((m: any) => {
                            ...
                            return (
                              <tr
                                key={m.month}
                                className={`${rowClass} hover:bg-[#F2F2F2]/50 transition-colors`}
                              >
                                <td
                    ...
                    <div className="px-4 py-2 bg-[#F2F2F2] border-t border-[#9EB3A8]/10 text-[10px] text-[#9EB3A8]">
                      <span className="text-cyan-600">Holding BTC</span> = BTC for capital
                      reconstitution &nbsp;|&nbsp;
                      <span className="text-[#E8A838]">Mining BTC</span> = Capitalization from
                      mining surplus &nbsp;|&nbsp;
            ...
            (() => {
              ...
              const mgmtFeesChartData = (() => {
                ...
                return baseCommercial.management_fees_monthly.map((_: number, t: number) => {
                  ...
                  for (const s of scenarios) {
                    const fees =
                      runData.scenario_results[s]?.commercial?.management_fees_monthly || []
                    row[s] = fees[t] || 0
                  }
                  return row
                })
                ...
                <div className="space-y-6">
                      ...
                      <div className={`${CARD} p-4`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-mono text-[#0E0F0F]">
                              {formatUSD(
                                baseComm.upfront_fee_breakdown?.holding_deduction_usd || 0,
                              )}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                  ...
                  {(() => {
                    const hasPerformanceFee = scenarios.some(
                      (s) =>
                        (runData.scenario_results[s]?.commercial?.performance_fee_usd || 0) > 0,
                    )
                    ...
                    return (
                      <div className={`${CARD} p-4`}>
                        ...
                        <p className="text-[10px] text-[#9EB3A8] mb-4">
                          Performance fee is calculated on the capitalization overhead (value above
                          initial mining investment)
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {scenarios.map((s) => {
                            const comm = runData.scenario_results[s]?.commercial
                      ...
                      <tbody className="divide-y divide-[#9EB3A8]/10">
                        <tr>
                          ...
                          {scenarios.map((s) => (
                            <td key={s} className="px-3 py-2 font-mono text-[#0E0F0F]">
                              {formatUSD(
                                runData.scenario_results[s]?.commercial?.upfront_fee_usd || 0,
                              )}
                            </td>
                          ))}
                        </tr>
                        <tr>
                          ...
                          {scenarios.map((s) => (
                            <td key={s} className="px-3 py-2 font-mono text-[#0E0F0F]">
                              {formatUSD(
                                runData.scenario_results[s]?.commercial
                                  ?.management_fees_total_usd || 0,
                              )}
                            </td>
                          ))}
                        </tr>
                        <tr>
                          ...
                          {scenarios.map((s) => (
                            <td key={s} className="px-3 py-2 font-mono text-[#0E0F0F]">
                              {formatUSD(
                                runData.scenario_results[s]?.commercial?.performance_fee_usd || 0,
                              )}
                            </td>
                          ))}
                        </tr>
                        <tr className="bg-amber-50">
                          <td className="px-3 py-2 font-semibold text-[#E8A838]">
                            Total Commercial Value
                          </td>
                          {scenarios.map((s) => (
                            <td
                              key={s}
                              className="px-3 py-2 font-mono font-semibold text-[#E8A838]"
                            >
                              {formatUSD(
                                runData.scenario_results[s]?.commercial
                                  ?.total_commercial_value_usd || 0,
                              )}
                            </td>
                          ))}
                        </tr>
          ...
          {viewTab === 'waterfall' &&
            (() => {
              const activeScenario = scenarios.includes(waterfallScenario)
                ...
                : scenarios[0]
              const waterfall: any[] =
                runData.scenario_results[activeScenario]?.mining_bucket?.monthly_waterfall || []
              const _miningMetrics =
                runData.scenario_results[activeScenario]?.mining_bucket?.metrics || {}
              const decision =
                runData.scenario_results[activeScenario]?.aggregated?.decision || 'PENDING'
              const reasons =
                runData.scenario_results[activeScenario]?.aggregated?.decision_reasons || []
              const holdingSellMonth =
                runData.scenario_results[activeScenario]?.btc_holding_bucket?.metrics?.sell_month
              ...
              return (
                <div className="space-y-5">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 text-xs flex-wrap">
                      ...
                      <span className="text-[#9EB3A8]">
                        ...
                        <span className="text-green-600 font-semibold">{greenMonths}</span> healthy
                        out of {totalMonths} months
                      </span>
                      <span className="text-[#9EB3A8]">|</span>
                  ...
                  <div className={`${CARD} p-4`}>
                    ...
                    <div className="text-[11px] text-[#9EB3A8] space-y-1 leading-relaxed">
                      <p>
                        Each month, BTC is produced by the mining fleet and allocated in strict
                        priority order:
                      </p>
                      <ol className="list-decimal list-inside space-y-0.5 pl-2">
                        <li>
                          <span className="text-orange-600 font-medium">OPEX</span> — Sell BTC to
                          cover electricity, hosting, and maintenance costs
                        </li>
                        <li>
                          <span className="text-green-600 font-medium">Yield</span> — Distribute
                          surplus as yield, capped at{' '}
                          <span className="text-[#0E0F0F] font-semibold">8% APR</span> (base) or{' '}
                          <span className="text-[#0E0F0F] font-semibold">12% APR</span> (once BTC
                          holding target is hit)
                        </li>
                        <li>
                          <span className="text-cyan-600 font-medium">Capitalization</span> —
                          Remaining BTC builds the capitalization / upside bucket
                        </li>
                      </ol>
                      <p className="mt-2">
                        A month is <span className="text-red-600 font-medium">DEFICIT (RED)</span>{' '}
                        if BTC produced {'<'} 95% of OPEX requirements. If {'>'} 20% of months are
                        deficit, the product is{' '}
                        <span className="text-red-600 font-medium">BLOCKED</span>.
                      </p>
                      <p>
                        <span className="text-[#96EA7A] font-medium">Capital reconstitution</span>{' '}
                        is handled by the BTC Holding bucket when the target price is hit, not by
                        mining.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className={`${CARD} p-4`}>
                    ...
                    <p className="text-[10px] text-[#9EB3A8] mt-1">
                      Dashed line = total BTC produced. Stacked bars = how it was allocated. When
                      bars fall short of the line, the month is in deficit.
                    </p>
                  </div>
                  ...
                  <div className={`${CARD} overflow-hidden`}>
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          onClick={() =>
                            exportAsCSV(
                              waterfall,
                              `waterfall-${activeScenario}-${selectedRunId.slice(0, 8)}.csv`,
                            )
                          }
                        >
                          CSV
                    ...
                    <div className="px-4 py-2 bg-[#F2F2F2] border-t border-[#9EB3A8]/10 text-[10px] text-[#9EB3A8]">
                      ...
                      RED = Deficit month (BTC produced {'<'} 95% of OPEX) &nbsp;
                      <span className="inline-block w-2 h-2 rounded-full bg-green-500 mr-1 align-middle" />{' '}
                      GREEN = OPEX covered, yield + capitalization distributed &nbsp;|&nbsp;
                      Threshold: {'>'} 20% RED months → BLOCKED &nbsp;|&nbsp;
                      <span className="text-[#96EA7A] font-medium">Green APR</span> = bonus yield
                      active (BTC holding target hit)
                    </div>
                  </div>
                </div>
              )
