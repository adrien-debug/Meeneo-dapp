...
function VaultPositionCard({ deposit, onNavigate }: { deposit: UserDeposit; onNavigate: (slug: string) => void }) {
  ...
  const daysRemaining = Math.max(0, Math.ceil((deposit.maturityTimestamp - Date.now() / 1000) / 86400))
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-caption font-semibold px-2.5 py-1 rounded-full ${STATUS_BG[deposit.lockStatus]} ${getLockStatusColor(deposit.lockStatus)}`}>
        ...
        <div className="mb-4">
          ...
          <div className="h-2 bg-[#F2F2F2] rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${deposit.progressPercent}%`, backgroundColor: statusColor }} />
          </div>
        </div>
        ...
        {deposit.pendingYield > 0 && (
          <div className="flex items-center justify-between p-4 bg-[#96EA7A]/5 rounded-xl border border-[#96EA7A]/15">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onNavigate(vault.slug) }}
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {activeDeposits.map(d => (
                  <VaultPositionCard key={d.id} deposit={d} onNavigate={(slug) => router.push(`/vault/${slug}`)} />
                ))}
              </div>
            </div>
          )}
          ...
          {MOCK_USER_DEPOSITS.length === 0 && (
            <div className={`${CARD} p-16 text-center`}>
              <div className="w-16 h-16 rounded-2xl bg-[#96EA7A]/10 flex items-center justify-center mx-auto mb-5">
                <svg className="w-8 h-8 text-[#96EA7A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                </svg>
              </div>
              <p className="text-lg font-bold text-[#0E0F0F] mb-2">No positions yet</p>
              <p className="text-sm text-[#9EB3A8] mb-6 max-w-xs mx-auto">Subscribe to a vault to start earning yield on your assets</p>
              <button
