              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-sm text-[#9EB3A8]">
                    Monitor TVL, distributions, and protocol health
                  </p>
                </div>
                <div className="flex items-center gap-2 self-start sm:self-auto">
                  <button
                    onClick={() =>
                      setSelectedAction(selectedAction === 'distribute' ? null : 'distribute')
                    }
                  ...
                  <button
                    onClick={() =>
                      setSelectedAction(selectedAction === 'rebalance' ? null : 'rebalance')
                    }
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-px bg-[#9EB3A8]/10 rounded-xl overflow-hidden">
                {[
                  ...
                  { label: 'Total Deposits', value: String(stats.totalDeposits) },
                  {
                    ...
                    value: fmtUsd(stats.totalFeesCollected),
                    accent: true,
                  },
                  {
                    label: 'Yield Distributed',
                    value: fmtUsd(stats.totalYieldDistributed),
                    accent: true,
                  },
                ].map((kpi) => (
                  ...
                  <div className="space-y-3 mb-5">
                    {vault.strategies.map((s) => {
                      ...
                      return (
                        <div
                          key={s.type}
                          className="flex items-center justify-between p-3 bg-[#F2F2F2] rounded-xl"
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-xs text-[#9EB3A8]">
                              {fmtUsd(actual)} / {fmtUsd(target)}
                            </span>
                            <span
                              className={`text-sm font-bold ${Math.abs(drift) < 2 ? 'text-[#96EA7A]' : 'text-[#E8A838]'}`}
                            >
                              {drift > 0 ? '+' : ''}
                              {drift.toFixed(1)}%
                            </span>
                ...
                <div key={strategy.type} className={`${CARD} p-5 hover:shadow-md transition-all`}>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-caption text-[#9EB3A8] font-medium">
                  ...
                  <div className="grid grid-cols-3 gap-px bg-[#9EB3A8]/10 rounded-xl overflow-hidden mb-4">
                    ...
                    <div className="bg-white px-3 py-2.5">
                      ...
                      <p className="text-sm font-black" style={{ color: strategy.color }}>
                        {fmtPercent(apy)}
                      </p>
                    </div>
                    <div className="bg-white px-3 py-2.5">
                      ...
                      <p
                        className={`text-sm font-black ${Math.abs(drift) < 2 ? 'text-[#96EA7A]' : 'text-[#E8A838]'}`}
                      >
                        {drift > 0 ? '+' : ''}
                        {drift.toFixed(1)}%
                      </p>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {strategy.protocols.map((p) => (
                      <span
                        key={p}
                        className="text-caption text-[#9EB3A8] bg-[#F2F2F2] px-2 py-0.5 rounded-full font-medium"
                      >
                        {p}
                      </span>
                    ))}
          ...
          <div className="grid grid-cols-12 gap-4 mb-5">
                    ...
                    <div key={a.name}>
                      ...
                      <div className="h-1 bg-[#F2F2F2] rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(a.value / vault.currentTvl) * 100}%`,
            ...
            <div className={`col-span-12 sm:col-span-6 lg:col-span-4 ${CARD} p-6 flex flex-col`}>
              ...
              <div className="flex-1 min-h-[140px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={[...PAST_DISTRIBUTIONS].reverse()}
              ...
              <div className="flex items-center justify-between pt-4 mt-2 border-t border-[#9EB3A8]/10">
                ...
                <span className="text-base font-black text-[#96EA7A]">
                  {fmtUsd(PAST_DISTRIBUTIONS.reduce((s, d) => s + d.amount, 0))}
                </span>
              </div>
            </div>
          </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className="w-12 h-1.5 rounded-full bg-white overflow-hidden">
                          <div
                            className={`h-full rounded-full ${s.latency < 30 ? 'bg-[#96EA7A]' : 'bg-[#9EB3A8]'}`}
                            style={{ width: `${Math.min(s.latency * 2, 100)}%` }}
                          />
                        </div>
                        <span className="text-xs font-mono text-[#9EB3A8] w-10 text-right">
                          {s.latency}ms
                        </span>
                ...
                <div className="grid grid-cols-2 gap-px bg-[#9EB3A8]/10 rounded-xl overflow-hidden mt-4">
                  ...
                  <div className="bg-white px-4 py-3 text-center">
                    ...
                    <p className="text-base font-black text-[#0E0F0F]">
                      {Math.round(
                        SYSTEM_HEALTH.reduce((s, h) => s + h.latency, 0) / SYSTEM_HEALTH.length,
                      )}
                      ms
                    </p>
                  </div>
                ...
                <div className="space-y-0">
                  {[
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-bold ${'accent' in stat && stat.accent ? 'text-[#96EA7A]' : 'text-[#0E0F0F]'}`}
                      >
                        {stat.value}
                      </span>
                    </div>
                  ...
                  {[
                    {
                      ...
                      bps: `${vault.fees.management * 100} bps`,
                    },
                    {
                      label: 'Performance',
                      value: `${vault.fees.performance}%`,
                      bps: `${vault.fees.performance * 100} bps`,
                    },
                    {
                      label: 'Exit',
                      value: `${vault.fees.exit}%`,
                      bps: `${vault.fees.exit * 100} bps`,
                    },
                    {
                      label: 'Early Exit',
                      value: `${vault.fees.earlyExit}%`,
                      bps: `${vault.fees.earlyExit * 100} bps`,
                      warn: true,
                    },
                  ].map((fee) => (
                    <div
              ...
              <div className="divide-y divide-[#9EB3A8]/5 flex-1 overflow-y-auto">
                {MOCK_RECENT_TRANSACTIONS.map((tx) => {
                  ...
                  return (
                    <div
                      key={tx.id}
                      className="flex items-center justify-between px-6 py-4 hover:bg-[#F2F2F2]/50 transition-colors"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${c.bg}`}
                        >
                          <svg
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {tx.strategy && (
                              <span className="text-caption text-[#9EB3A8] bg-[#F2F2F2] px-1.5 py-0.5 rounded-full">
                                {vault.strategies.find((s) => s.type === tx.strategy)?.label}
                              </span>
                            )}
                          </div>
                      ...
                      <div className="text-right shrink-0 ml-3">
                        <p
                          className={`text-sm font-bold ${tx.type === 'claim' || tx.type === 'distribute' ? 'text-[#96EA7A]' : 'text-[#0E0F0F]'}`}
                        >
                          {tx.type === 'claim' || tx.type === 'distribute' ? '+' : ''}
                          {fmtUsd(tx.amount)}
                        </p>
